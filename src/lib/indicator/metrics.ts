...
export function computeSummonerMetrics(
	...
	const metrics: Metric[] = [
		...
		// KDA 관련
		{
			...
			label: "인분",
			value: kda,
			note: `킬/데스/어시 ${(totals.kills / games).toFixed(1)}/${(totals.deaths / games).toFixed(1)}/${(totals.assists / games).toFixed(1)}`,
			summonerName: summonerName,
		},
		{
			key: "kda_ratio",
			label: "KDA",
			value: kda,
			note: `킬+어시 / 데스 ${((totals.kills + totals.assists) / Math.max(1, totals.deaths)).toFixed(2)}%`,
			summonerName: summonerName,
		},
		...
		// 골드/CS 관련
		{
			...
			note: `10분 이전 골드량 ${timelineMetrics.earlyGoldAdvantage > 0 ? "우세" : "열세"}`,
			summonerName: summonerName,
		},
		{
			key: "cs_per_min",
			label: "15분 CS 차이",
			value: csPerMin,
			note: `미니언 + 정글몹 ${(timelineMetrics.csAt15 / games).toFixed(1)}`,
			summonerName: summonerName,
		},
		{
			key: "gold_per_min",
			label: "15분 골드 차이",
			value: goldPerMin,
			note: `전체골드 ${timelineMetrics.goldDiffAt15.toFixed(0)}`,
			summonerName: summonerName,
		},
		{
			key: "lane_advantage",
			...
			note: `레벨 ${timelineMetrics.levelDiffAt15 > 0 ? "+" : ""}${timelineMetrics.levelDiffAt15.toFixed(1)}`,
			summonerName: summonerName,
		},
		{
			...
			note: `(킬+어시)/팀 전체 킬 ${timelineMetrics.killParticipation.toFixed(0)}%`,
			summonerName: summonerName,
		},
		...
		// 데미지/생존력
		{
			...
			note: `킬 / 게임수 ${((totals.kills / games) * 100 * 0.01).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "damage_share",
			...
			note: `어시 / 게임수 ${((totals.assists / games) * 100 * 0.01).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "death_count",
			...
			note: `데스 / 게임수 ${(100 - (totals.deaths / games) * 10).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "solo_kill",
			...
			note: `솔로킬 ${(timelineMetrics.soloKills * 10).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "solo_kill_ratio",
			...
			note: `솔로데스 ${Math.max(0, 100 - timelineMetrics.soloDeaths * 20).toFixed(0)}%`,
			summonerName: summonerName,
		},
		...
		// 평점/등급
		{
			...
			note: `cs / 게임수 ${Math.min(100, (totals.cs / games / 200) * 100).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "gold_share",
			...
			note: `골드 / 게임수 ${Math.min(100, (totals.gold / games / 15000) * 100).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "team_damage",
			...
			note: `딜량 / 게임수 ${Math.min(100, (totals.damage / games / 25000) * 100).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "team_tanking",
			...
			note: `받은딜량 / 게임수 ${Math.min(100, (totals.damageTaken / games / 30000) * 100).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "gold_efficiency",
			...
			note: `데미지 / 골드량 ${Math.min(100, (totals.damage / Math.max(1, totals.gold)) * 100000).toFixed(0)}%`,
			summonerName: summonerName,
		},
		...
		// 고급 지표
		{
			...
			note: `전체딜량 / 게임수 ${(100 - totals.damage / games / 1000).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			...
			note: `전체받은딜량 / 게임수 ${(totals.damageTaken / games / 1000).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "win_rate",
			label: "킬 관여율",
			value: `${winRate}%`,
			note: `${totals.wins}승 ${games - totals.wins}패`,
			summonerName: summonerName,
		},
		{
			key: "vision_score",
			...
			note: `시야점수 / 게임수 ${Math.min(100, (totals.visionScore / games / 50) * 100).toFixed(0)}%`,
			summonerName: summonerName,
		},
		{
			key: "ward_efficiency",
			...
			note: `와드제거수 / 와드설치수 ${Math.min(100, (totals.wardsKilled / Math.max(1, totals.wardsPlaced)) * 50).toFixed(0)}%`,
			summonerName: summonerName,
		},
	];
